...
func (s *internalCircleSliceView) growIfNecessary(slice []Circle,
	requiredLen int) (*internalCircleSliceHeader, error) {
	var tVar Circle
	tSize := unsafe.Sizeof(tVar)
	requiredSizeInBytes := requiredLen * int(tSize)
	sliceHdr := (*internalCircleSliceHeader)(unsafe.Pointer(&slice))
...
func (s *internalCircleSliceView) makeGoSlice(len int) (*internalCircleSliceHeader, error) {
	valueSlice, allocErr := s.state.makeSlice(len)
	if allocErr != nil {
		return nil, allocErr
	}
	sliceRef := s.state.alloc.ToRef(valueSlice.data)
	sliceHdr := internalCircleSliceHeader{
...
type internalCircleSliceHeader struct {
	Data uintptr
	Len  int
	Cap  int
}
